...
“真实 PR 评测”工作台支持批量收录已有审查、固定提交的基线/候选对比、调参与验收划分、
参考缺陷双人确认、问题复核、变更代码快照和聚合报告。未知费用、未复核与分歧会单独显示。
详见 [评测工作台契约](docs/contracts/evaluation-workbench.md)。真实效果数据需要实际人工复核后积累。

团队管理页现支持成员启停、角色与仓库范围分配、版本冲突检测和会话撤销；
仓库策略支持目标分支、知识文档范围、审批负责人和可选模型请求上限。
每个新任务保存自己的策略版本，详细语义见 [团队管理契约](docs/contracts/team-management.md)。
