  ...
  return <section className="workspace-surface evaluation-card evaluation-report-card" aria-label="两份审查效果对比">
    ...
    {report && <>
      ...
      <div className="evaluation-table-wrap"><table>
        <thead><tr><th>指标</th><th>第一份审查</th><th>第二份审查</th><th>统计范围</th></tr></thead>
        <tbody>
          <tr><td>有效问题比例</td><td>{evaluationPercent(report.baseline.precision)}</td><td>{evaluationPercent(report.candidate.precision)}</td><td>{report.quality_pairs} 对，两份均完成核对</td></tr>
          ...
          <tr><td>定位正确比例</td><td>{evaluationPercent(report.baseline.location_accuracy)}</td><td>{evaluationPercent(report.candidate.location_accuracy)}</td><td>核对人明确判断过位置的问题</td></tr>
          <tr><td>重复问题比例</td><td>{evaluationPercent(report.baseline.duplicate_rate)}</td><td>{evaluationPercent(report.candidate.duplicate_rate)}</td><td>两份均完成核对的配对样本</td></tr>
          ...
          <tr><td>平均估算费用</td><td>{money(report.baseline.mean_estimated_cost_usd)}</td><td>{money(report.candidate.mean_estimated_cost_usd)}</td><td>{report.priced_pairs} 对，两份费用均已记录</td></tr>
