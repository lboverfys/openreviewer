...
export default function ProfileActivationPanel({ id, repository, revision, onDone, onCancel, onError }: {
  ...
  return <section className="workspace-surface ws-editor" aria-label="方案启用质量确认">
    <div className="ws-editor-heading"><div><h2>确认启用审查方案</h2><p>{repository} · 新任务使用此方案，已有任务保持原版本。</p></div><Button variant="outline" type="button" disabled={busy} onClick={() => setRefresh(value => value + 1)}>重新读取质量依据</Button></div>
    <WorkspaceSection title="验收依据" description="比较同一提交下的基线与候选结果。">
      <label>验收评测集<NativeSelect disabled={busy} value={dataset} onChange={event => setDataset(event.target.value)}><option value="">暂无可用评测，记录理由后人工启用</option>{datasets.data?.items.filter(item => item.repository.toLowerCase() === repository.toLowerCase()).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</NativeSelect></label>
    ...
    {quality ? <>
      ...
      {quality.report && <div className="team-table-wrap"><Table><TableHeader><TableRow><TableHead>指标</TableHead><TableHead>当前方案</TableHead><TableHead>候选方案</TableHead></TableRow></TableHeader><TableBody>
        <TableRow><TableCell>有效问题比例</TableCell><TableCell>{percent(quality.report.baseline.precision)}</TableCell><TableCell>{percent(quality.report.candidate.precision)}</TableCell></TableRow>
        <TableRow><TableCell>已知缺陷找回率</TableCell><TableCell>{percent(quality.report.baseline.recall)}</TableCell><TableCell>{percent(quality.report.candidate.recall)}</TableCell></TableRow>
        <TableRow><TableCell>平均估算费用</TableCell><TableCell>{cost(quality.report.baseline.mean_estimated_cost_usd)}</TableCell><TableCell>{cost(quality.report.candidate.mean_estimated_cost_usd)}</TableCell></TableRow>
      </TableBody></Table></div>}
    ...
    <form onSubmit={event => void submit(event)}><fieldset disabled={busy || !quality}>
      ...
      <div className="ws-form-actions"><Button variant="default" className="ws-primary" type="submit">确认启用方案</Button><Button variant="outline" type="button" onClick={onCancel}>取消</Button><span className="ws-hint">启用时再次核对评测与策略版本</span></div>
    </fieldset></form>
  </section>;
}
