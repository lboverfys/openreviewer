...
export default function TeamPage({ onSignedOut, initialRepository, initialSection }: { onSignedOut: (message?: string) => void; initialRepository?: string; initialSection?: string }) {
  ...
  return <main className="workspace-page team-page">
    <WorkspaceHeader title="项目与成员" icon="team" description="项目设置决定审查哪些代码，成员权限决定谁能查看、核对和发布结果。" actions={member || repository
      ...
      : <Button variant="outline" type="button" disabled={busy || page.loading} onClick={() => {setError(""); void page.refresh();}}>刷新列表</Button>} />
    {!member && !repository && <nav className="team-tabs" aria-label="团队管理分类">
      {([["repositories", "项目设置"], ["members", "成员权限"], ["audits", "操作记录"]] as const).map(([key, label]) =>
        <Button variant="outline" key={key} type="button" aria-pressed={tab === key} disabled={busy} onClick={() => changeTab(key)}>{label}</Button>,
      )}
    </nav>}
    ...
    {!member && !repository && !connecting && <section className="team-card">
      <div className="team-toolbar">
        ...
        {tab !== "audits" && <Button variant="default" type="button" className="team-primary" disabled={busy} onClick={() => {
          setError(""); setMessage("");
      ...
      <div className="team-table-wrap">
        {tab === "repositories" && <Table>
          <TableHeader><TableRow><TableHead>仓库</TableHead><TableHead>接入状态</TableHead><TableHead>新任务</TableHead><TableHead>目标分支</TableHead><TableHead>审批负责人</TableHead><TableHead>请求上限</TableHead><TableHead>版本</TableHead><TableHead>操作</TableHead></TableRow></TableHeader>
          <TableBody>{repositories.data?.items.map((item) => <TableRow key={item.id}>
            <TableCell><strong>{item.repository}</strong></TableCell><TableCell>{item.connected_at ? <><WorkspaceBadge tone="success">已接通</WorkspaceBadge><small>上次检查：{formatDate(item.connected_at)}</small><small>{item.policy.enabled ? "新 PR 自动触发审查" : "已暂停自动审查"}</small></> : <><WorkspaceBadge>尚未检查接入</WorkspaceBadge><small>{item.connection_error ?? "已保存策略，请检查授权和访问能力"}</small></>}</TableCell><TableCell><WorkspaceBadge tone={item.policy.enabled ? "success" : "neutral"}>{item.policy.enabled ? "接收" : "暂停"}</WorkspaceBadge></TableCell>
            <TableCell>{item.policy.target_branches?.join("、") || "全部"}</TableCell><TableCell>{item.policy.approver || "按角色审批"}</TableCell>
            <TableCell>{item.policy.max_model_requests == null ? "不限制" : item.policy.max_model_requests + " 次"}</TableCell><TableCell>v{item.revision}</TableCell>
            <TableCell><Button variant="outline" type="button" aria-label={"编辑仓库 " + item.repository} disabled={busy} onClick={() => {setRepository(item); setMessage("");}}>编辑</Button><Button variant="outline" disabled={busy} onClick={() => {
            ...
            }}>检查接入</Button>{item.connection_error && <Button variant="outline" onClick={() => setConnecting(item)}>补充授权</Button>}</TableCell>
          </TableRow>)}</TableBody>
        </Table>}
        {tab === "members" && <Table>
          <TableHeader><TableRow><TableHead>用户名</TableHead><TableHead>角色</TableHead><TableHead>可见资源</TableHead><TableHead>状态</TableHead><TableHead>版本</TableHead><TableHead>操作</TableHead></TableRow></TableHeader>
          <TableBody>{members.data?.items.map((item) => <TableRow key={item.username}>
            <TableCell><div className="team-member-identity"><span className="team-member-avatar">{item.username.slice(0, 1).toUpperCase()}</span><strong>{item.username}</strong>{item.username.toLowerCase() === administrator.toLowerCase() && <small>初始管理员</small>}</div></TableCell><TableCell>{roleLabels[item.role]}</TableCell>
            <TableCell>{item.scope.unrestricted ? "全部仓库" : [...(item.scope.repositories ?? []), ...(item.scope.organizations ?? []).map((name) => name + "/*")].join("、") || (item.scope.installation_ids?.length ? "指定 App 安装范围" : "未分配")}</TableCell>
            <TableCell><WorkspaceBadge tone={item.enabled ? "success" : "neutral"}>{item.enabled ? "启用" : "停用"}</WorkspaceBadge></TableCell><TableCell><span className="ws-chip">v{item.revision}</span></TableCell>
            <TableCell><Button variant="outline" type="button" aria-label={"编辑成员 " + item.username} disabled={busy} onClick={() => {setMember(item); setMessage("");}}>编辑</Button></TableCell>
          </TableRow>)}</TableBody>
        </Table>}
        {tab === "audits" && <Table>
          <TableHeader><TableRow><TableHead>时间</TableHead><TableHead>操作人</TableHead><TableHead>操作</TableHead><TableHead>对象</TableHead><TableHead>变更内容</TableHead></TableRow></TableHeader>
          <TableBody>{audits.data?.items.map((item) => <TableRow key={item.id}>
            <TableCell>{formatDate(item.occurred_at)}</TableCell><TableCell>{String(item.payload.actor ?? "")}</TableCell>
            <TableCell>{item.event_type === "team.member.updated" ? "更新成员" : item.event_type === "team.repository.connection_checked" ? "检查仓库接入" : "更新仓库策略"}</TableCell>
            <TableCell>{String(item.payload.target ?? "")}</TableCell>
            <TableCell><DetailDialog><summary>查看详情</summary><pre>{JSON.stringify(item.payload, null, 2)}</pre></DetailDialog></TableCell>
          </TableRow>)}</TableBody>
        </Table>}
      </div>
    ...
    <form onSubmit={submit}>
      <fieldset disabled={busy}>
        <WorkspaceSection title="账号信息" description="分配角色，维护账号状态与登录密码。"><div className="team-form-grid">
          ...
          <label>角色<NativeSelect disabled={initialAdmin} value={role} onChange={(event) => {setRole(event.target.value as AccessRole); setUnrestricted(event.target.value === "administrator");}}>{Object.entries(roleLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</NativeSelect></label>
        ...
        <div className="ws-form-actions is-sticky"><Button variant="default" type="submit" className="team-primary">{busy ? "正在保存…" : "保存成员"}</Button><Button variant="outline" type="button" onClick={onCancel}>取消编辑</Button></div>
      </fieldset>
    </form>
  </section>;
}
  ...
  return <section className="team-card team-editor ws-editor" aria-label="仓库策略编辑">
    ...
    <form onSubmit={submit}><fieldset disabled={busy}>
      <WorkspaceSection title="审查范围" description="选择接入仓库、目标分支和团队知识。">
        ...
        <label>知识规则<NativeSelect value={knowledgeMode} onChange={event => setKnowledgeMode(event.target.value)}><option value="inherit">继承已启用知识库</option><option value="selected">仅使用指定文档</option><option value="none">不使用知识库</option></NativeSelect></label>
      ...
      <Button variant="default" className="ws-primary" type="submit" disabled={busy}>保存预算与项目设置</Button>
      </div></DetailDialog>
      ...
      <div className="ws-form-actions is-sticky"><Button variant="default" type="submit" className="team-primary">{busy ? "正在保存…" : "保存项目设置"}</Button><Button variant="outline" type="button" onClick={onCancel}>取消编辑</Button><span className="ws-hint">保存前会检查是否有其他人的修改</span></div>
    </fieldset></form>
  </section>;
}
