...
function DatasetList({ canEdit,reviewRunId,onError }: {canEdit:boolean;reviewRunId?:string;onError:(error:unknown)=>void}) {
  ...
  return <>
    <section className="evaluation-card">
      <div className="evaluation-toolbar"><div><h2>评测集</h2><p>按仓库管理审查基线与候选方案。</p></div><div className="ws-actions"><label className="evaluation-check"><input type="checkbox" checked={archived} onChange={event=>setArchived(event.target.checked)}/>包含已归档</label>
        <button type="button" disabled={page.loading} onClick={()=>void page.refresh()}>刷新列表</button>
        {canEdit && <button type="button" className="evaluation-primary" onClick={()=>setCreating(true)}>创建评测集</button>}</div></div>
      <div className="evaluation-table-wrap"><table><thead><tr><th>名称 / 仓库</th><th>PR 样本数</th><th>创建人</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>{page.data?.items.map(item=><tr key={item.id}><td><strong>{item.name}</strong><small>{item.repository}</small></td><td className="ws-numeric">{item.case_count}</td><td>{item.created_by}</td><td><WorkspaceBadge tone={item.archived_at ? "neutral" : "accent"}>{item.archived_at?"已归档":"进行中"}</WorkspaceBadge></td>
          <td><button type="button" onClick={()=>open(item.id)}>打开评测集</button></td></tr>)}</tbody></table></div>
      {!page.loading && page.data?.items.length===0 && <WorkspaceEmpty title="尚无评测集" description="选择已完成的审查建立基线，再收录同一提交的候选结果。" />}
