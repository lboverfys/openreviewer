...
export default function EvaluationSourcePicker({ datasetId, caseId, selected, onSelected, onError, disabled = false, excludeRunId, single = false }: {
  ...
  return <section className="evaluation-picker" aria-label="选择审查运行">
    <div className="evaluation-toolbar"><strong>选择已完成的审查运行</strong><span>已选择 {selected.length} 条</span>
    ...
    <div className="evaluation-table-wrap"><table>
      ...
      <tbody>{page.data?.items.map(item => <tr key={item.review_run_id}>
    ...
    {!page.loading && page.data?.items.length === 0 && <p className="evaluation-empty">暂无符合条件的运行。可先完成审查，或为相同提交重新审查后收录。</p>}
