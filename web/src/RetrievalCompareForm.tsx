...
export default function RetrievalCompareForm({indexId, onSaved, onError}: {indexId: string; onSaved: () => void; onError: (error: unknown) => void}) {
  ...
  return <form onSubmit={event => {
    ...
    <fieldset disabled={busy}><legend>需要比较的方式</legend>{Object.entries(retrievalStrategyLabels).map(([key, label]) => <label key={key}><input type="checkbox" checked={strategies.includes(key as RetrievalStrategy)} onChange={event => setStrategies(current => event.target.checked ? [...current, key as RetrievalStrategy] : current.filter(item => item !== key))}/>{label}</label>)}</fieldset>
    <p>来源如实记录为单人标注。向量和精排策略可能调用已配置的模型，并受调用开关与预算限制；策略降级时不会生成误导性的对比分数。</p>
    <button className="primary" disabled={busy || !indexId || strategies.length < 2}>{busy ? "正在比较…" : "按参考代码运行对比"}</button>
  </form>;
}
