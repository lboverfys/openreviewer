  ...
  return <aside className="retrieval-card retrieval-library">
    ...
    {selected ? <>
      ...
      <ol className="retrieval-stages" aria-label="索引阶段">
        ...
        <li className={selected.vector_status === "ready" ? "done" : pending ? "working" : ""}><span className="retrieval-stage-dot" /><div><strong>向量补全</strong><small>{pending && selected.lexical_ready ? selected.status === "building" ? "正在补全" : "等待补全" : vectorLabels[selected.vector_status ?? "pending"]} · {selected.vector_count ?? 0} / {selected.chunk_count} 块</small></div></li>
