...
export default function UserManual({ initialTopic, onClose }: { initialTopic: Topic; onClose: () => void }) {
  ...
  const content: Record<Topic, ReactNode> = {
    ...
    usage: <>
      ...
      <dl><dt>已估算费用</dt><dd>根据服务商返回的 Token 和请求发生时保存的单价计算。在请求明细中打开“计费依据”查看当次价格和用量。</dd>
        <dt>待确认费用</dt><dd>缺少完整用量、普通单价或缓存单价时分别显示原因；历史记录没有保存原因时标为历史原因未记录，不能当作免费。</dd>
        <dt>预算提醒</dt><dd>接近预算时提醒管理员检查；具体限制以项目设置为准。</dd></dl>
      <p>请求状态、用量和费用分别展示。HTTP 成功或请求结束不代表费用已确认；已估算金额仍需与服务商账单核对。补填当前价格不会改写历史记录。</p>
