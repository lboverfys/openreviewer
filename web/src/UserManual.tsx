...
export default function UserManual({ initialTopic, onClose }: { initialTopic: Topic; onClose: () => void }) {
  ...
  const content: Record<Topic, ReactNode> = {
    ...
    evaluations: <>
      ...
      <ol><li>选择已经完成的真实审查，建立一组评测样本。</li>
        <li>逐条复核候选问题，并补充代码中实际存在的参考问题。没有候选问题，也应检查漏报。</li>
        <li>只有一组审查也能查看有效问题、误报和复核进度。</li>
        <li>比较模型或配置时，再加入同一个 PR、同一个提交的另一条独立审查，查看两组结果的对比。</li></ol>
      <p>“原结果”是比较起点，“新结果”是要评估的方案。无法配对时，页面不会编造改进率。AI 账号的复核仍是 AI 复核，不能当作独立人工验证。</p>
