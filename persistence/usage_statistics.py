...
def request_statistics_statement(predicate, dimensions: tuple[str, ...] = ()):
    ...
    statement = select(
        ...
        *(func.min(case((row.position * 100 >= row.timed_count * percentile, row.duration_ms))).label(f"p{percentile}_duration_ms") for percentile in (50, 95)),
