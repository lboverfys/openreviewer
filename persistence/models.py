...
class EvaluationDatasetRecord(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_evaluation_datasets_archive_created", "archived_at", "created_at", "id"),
        Index("ix_evaluation_datasets_repository_created", "repository_key", "created_at", "id"),
    )
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
...
class EvaluationCaseRecord(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("kind IN ('normal','known_defect','cross_file')", name="kind_value"),
        CheckConstraint("revision > 0", name="revision_positive"),
    ...
    reference_reviews: Mapped[list[dict[str, object]]] = mapped_column(JSON, nullable=False, default=list)
    reference_status: Mapped[str] = mapped_column(String(16), nullable=False, default="pending")
...
class EvaluationObservationRecord(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("revision > 0", name="revision_positive"),
        CheckConstraint("finding_count BETWEEN 0 AND 500", name="finding_count_bounded"),
        CheckConstraint(
    ...
    case_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("evaluation_cases.id", ondelete="CASCADE"), nullable=False,
    )
    ...
    estimated_cost_microusd: Mapped[int | None] = mapped_column(BigInteger)
    ballots: Mapped[list[dict[str, object]]] = mapped_column(JSON, nullable=False, default=list)
    metrics: Mapped[dict[str, int]] = mapped_column(JSON, nullable=False, default=dict)
    assessment_status: Mapped[str] = mapped_column(String(16), nullable=False, default="pending")
    revision: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
