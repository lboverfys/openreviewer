...
- 当前由人工批准后发布 GitHub Check、通过历史评测门槛的高置信行内评论和 PR 汇总评论；
...
分批、版本化 RAG、可恢复批次、用量成本、Finding 持久化、跨提交生命周期、自动源码证据核验，
以及批准后幂等发布 Check、行内评论和 PR 汇总评论。真实模型评测与跨仓库趋势分析仍待接入。
...
8. 当前 PR 汇总评论、行内评论和 AI Check 都不阻止合并，先积累真实 PR 的有效问题和误报数据。
...
| GitHub 适配器 | 读取 PR/CI/Blob，创建或更新 Check、批量行内评论和 PR 汇总评论 |
...
只有同时满足以下条件的 Finding 才能成为行内评论候选（自动源码核验和人工 Finding 裁决是
两道独立门）：
...
当前人工裁决有效的候选进入 Check 和 PR 汇总；只有通过历史评测门槛、位置与 SHA 校验且置信度
足够的候选进入行内评论，GitHub 拒绝位置时自动降级。无法稳定定位的问题进入 Check Summary，
避免重复评论和刷屏。模型输出的置信度只是一项
参考，不能绕过证据、位置和评测门槛。
...
- GitHub App 以 Pull requests 和 Checks 读写权限发布评论与 Check，Metadata、Contents 和 Commit
  statuses 保持只读；不申请 Contents 写权限。
...
超限后进入明确的部分覆盖或人工确认状态，不能静默漏审。每个 Worker 进程保持单执行槽，
部署可通过多个副本横向扩展；是否继续扩容仍需由真实数据中的队列延迟、数据库负载和模型限流
共同验证。
...
1. Shadow mode：人工确认后可先只发布 PR 汇总评论，不阻止合并；Check 和行内评论按风险域
   准入策略逐步开放。
...
| 已完成最小闭环 | Review Unit、固定四 Agent DAG、RAG、人工批准和 PR 汇总评论 |
| 已完成发布增强 | 定位/裁决分离、GitHub Check、行内评论和同 PR 跨提交生命周期 |
...
- 自动源文件证据回读的边界样本、失败原因和人工裁决标注规范。
- 跨仓库、跨 PR 的 Finding 聚类和长期趋势策略。
