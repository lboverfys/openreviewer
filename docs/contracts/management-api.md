...
当前管理界面支持配置管理员与数据库管理的团队成员。成员和仓库策略见
[团队管理契约](team-management.md)。除 `/healthz` 外，下列接口都通过同源
...
旧额外账号配置在 `OPENREVIEWER_AUTH_USERS_FILE` 指向的 JSON 数组中，首次使用新版认证时
批量导入数据库；此后从“团队管理”页面维护，不再通过文件覆盖。旧配置每项包含
...
验证会话；流运行期间每两秒再次查询服务端会话记录和成员版本，并单独检查签名载荷中的绝对到期时间。
