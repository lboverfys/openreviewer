...
知识库管理、内容边界、数据库模型、HTTP 接口和错误语义以
[knowledge-management.md](knowledge-management.md) 为准。Worker 镜像仍内置
`knowledge/*.md`，但这些文件只在数据库首次为空时作为 `system:seed` 导入；生产运行期间
以 PostgreSQL 中的启用、未归档当前版本为事实来源。

每次审查进入 Agent 循环前只读取一次有界知识快照（最多 128 份、单份 512 KiB、总计 5 MiB），
安全、规范、逻辑和汇总 Agent 使用同一快照做确定性词法匹配，并结合各自职责词与最多 32 个
变更文件名查询，每路最多引用 8 条。中文连续文本会补充相邻双字词，内容按 Markdown 标题
切块；引用带 `source`、标题、摘录和内容版本前 16 位，进入 Prompt 和结构化事件。

知识库内容与仓库规则一样是不可信输入，不能扩大权限、触发工具或绕过输出 Schema。任务
开始后产生的知识库修改只影响后续任务，不会改变当前任务已经取得的快照。
