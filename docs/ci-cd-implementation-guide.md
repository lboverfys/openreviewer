...
`test` 和 `web-test` 可以并行；容器冒烟验证等待两者成功，`publish` 再等待容器验证，
`deploy` 最后等待 `publish` 成功。
...
python -m ruff check apps domain persistence services tests
python -m mypy apps domain persistence services
python -m pytest -q -W error --cov=apps --cov=domain --cov=persistence --cov=services
```

`-W error` 会把警告视为失败。CI 同时启动隔离的 PostgreSQL 16，执行 Alembic 迁移并验证
`FOR UPDATE SKIP LOCKED` 锁语义；其余快速 API/服务测试使用 SQLite。依赖由
`requirements.lock` 约束，CI 还会核对 OpenAPI/评测 Schema、运行录制输出回归和
`pip-audit`；Python 覆盖率门槛为 75%。
...
依赖按 `web/package-lock.json` 安装并使用 npm 缓存；单独的 CI 步骤执行
`npm audit --audit-level=high`。当前仍没有真实浏览器端到端测试，Vitest 覆盖率门槛为
语句/行 20%、函数 15%、分支 10%。
...
CI 会把构建步骤返回的两个 `sha256` manifest digest 写入受限发布包；服务器按 digest
拉取并核对实际镜像，再允许启动。SHA 标签仍不是 GHCR 强制不可覆盖，digest 清单用于
抵御标签被重新指向。

后端镜像使用 Python 3.12，以非 root 用户运行。Web 镜像先用 Node 构建 React，
再把静态文件放入非 root Nginx 镜像。构建启用 SBOM、来源证明，并对源码和发布镜像执行
Trivy 高危/严重漏洞扫描。
...
部署通道只传 SHA、固定白名单部署文件和 CI 生成的两个 digest 清单，不传服务器 `.env`、
密码或 TLS 私钥。服务器公钥应配置 forced command 和转发限制，使部署密钥不能直接取得
任意交互式 Shell。

服务器必须预先准备 Docker、部署入口、`/opt/openreviewer` 初始 release、真实 `.env`、
TLS 文件和镜像读取权限。发布包还带有 `helper-digests.env`，部署入口会在接触 Docker
和数据库前核对服务器上的 deploy/backup/restore 三份脚本；任何缺失、手工改动或版本漂移
都会直接拒绝发布。首次安装以及脚本变更后的第一次发布，管理员仍需单独更新服务器副本，
因为旧入口无法替自己安装新版本。
...
2. 接收不超过 1 MiB 的固定白名单部署包，并检查项目名、镜像变量、两个 digest 清单和服务名；
   先核对三份已安装运维脚本的 SHA-256。
3. 获取服务器文件锁，避免并发发布。
4. 从上一成功 release 复制 `.env`，只替换两个镜像地址。
5. 验证 Compose，按 CI digest 拉取并标记本次 API/Web 镜像，同时核对 RepoDigest。
6. 启动 PostgreSQL，先停止并确认旧版本 API/Worker/Web 已退出，再执行 `alembic upgrade head`；
   迁移失败或中途异常时保持应用服务停止，人工核对数据库状态后再恢复。
7. 原地替换 API、Worker 和 Web；Worker 副本数由 `OPENREVIEWER_WORKER_REPLICAS`
   经 `docker compose --scale` 显式控制。
...
├── releases/<SHA>/{compose.yml,image-digests.env,helper-digests.env,.env,release.info}
...
| `prometheus` | 指标采集和告警规则 | 仅内部 |
| `alertmanager` | 告警路由（可选外发） | 仅内部 |
| `grafana` | 只读监控面板 | 默认宿主机回环 |
...
| `deployment/deploy.sh`、`backup.sh`、`restore.sh` | 发布前由 `helper-digests.env` 校验；脚本变更后首次发布前需手工更新服务器副本 |
| `deployment/.env.example` | 只更新示例，不修改服务器真实 `.env` |
| Workflow | 后续运行按新规则执行，应重点审查权限和 Secret 使用 |

优先改进项：

1. 将基础镜像和第三方 Compose 镜像也固定为 digest，并定期更新镜像来源。
2. 增加真实浏览器端到端测试和部署后业务级冒烟（登录、创建任务、发布模拟）。
3. 为迁移增加线上影子演练，明确长事务、锁等待和向后兼容窗口。
4. 为发布失败增加外部通知，并持续演练有界 release/备份清理和恢复脚本。
5. 若要求近似零停机，再引入多副本 API、连接耗尽保护和流量切换机制。
