...
def test_aliyun_batch_order_is_restored_and_credentials_stay_in_header():
    ...
    client = AliyunRetrievalClient(RetrievalSettings(api_host="https://example.cn-beijing.maas.aliyuncs.com"), "test-key", httpx.Client(transport=httpx.MockTransport(handler)))
...
def test_vector_dimensions_duplicates_and_nonfinite_values_are_rejected():
    for rows in [
        ...
        client = AliyunRetrievalClient(RetrievalSettings(api_host="https://example.cn-beijing.maas.aliyuncs.com"), "test-key", httpx.Client(transport=httpx.MockTransport(lambda request, rows=rows: httpx.Response(200, json={"data": rows}))))
...
def test_quota_failure_exposes_only_safe_code_and_does_not_retry():
    ...
    def handler(request):
        ...
        return httpx.Response(403, json={"error": {"code": "insufficient_quota", "message": "test-key private text"}})
    client = AliyunRetrievalClient(RetrievalSettings(api_host="https://example.cn-beijing.maas.aliyuncs.com"), "test-key", httpx.Client(transport=httpx.MockTransport(handler)))
...
def test_429_retry_uses_bounded_backoff(monkeypatch):
    ...
    def handler(request):
        nonlocal calls
        calls += 1
        return httpx.Response(429, headers={"retry-after": "2"}, json={"code": "Throttling"})
    client = AliyunRetrievalClient(RetrievalSettings(api_host="https://example.cn-beijing.maas.aliyuncs.com"), "test-key", httpx.Client(transport=httpx.MockTransport(handler)))
    with pytest.raises(RetrievalError):
        client.embed(("one",))
...
def test_provider_host_does_not_accept_unrelated_or_private_endpoints():
    assert normalize_aliyun_host("https://a.cn-beijing.maas.aliyuncs.com/compatible-mode/v1") == "https://a.cn-beijing.maas.aliyuncs.com"
    for value in ["http://127.0.0.1", "https://evil.example", "https://maas.aliyuncs.com.evil.example", "https://a.cn-beijing.maas.aliyuncs.com/arbitrary"]:
...
def test_server_pause_switch_blocks_real_requests_before_transport(monkeypatch):
    ...
    calls = []
    http = httpx.Client(transport=httpx.MockTransport(lambda request: calls.append(request)))
    client = AliyunRetrievalClient(RetrievalSettings(api_host="https://example.cn-beijing.maas.aliyuncs.com"), "test-key", http)
